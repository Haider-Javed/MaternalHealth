...
from gemini_service import parse_audio_to_vitals, generate_precautions
...
class PredictRequest(BaseModel):
    Age: int = Field(..., ge=10, le=70, example=28)
    SystolicBP: int = Field(..., ge=60, le=220, example=140)
    DiastolicBP: int = Field(..., ge=40, le=140, example=90)
    BS: float = Field(..., ge=3.0, le=25.0, example=8.5)
    BodyTemp: float = Field(..., ge=95.0, le=106.0, example=98.6)
    HeartRate: int = Field(..., ge=40, le=180, example=75)
...
async def process_audio(file: UploadFile = File(...)):
    """
    Receive an audio recording (webm/wav), feed it to Gemini 1.5 Flash,
    and return the extracted 6 vitals as JSON.
    """
    audio_bytes = await file.read()
    mime_type = file.content_type or "audio/webm"

    try:
        vitals = await parse_audio_to_vitals(audio_bytes, mime_type)
    except Exception as exc:
        raise HTTPException(status_code=422, detail=f"Audio parsing failed: {exc}")
